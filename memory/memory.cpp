...
int alloc(v_address* addr, m_size size, m_pid pid) {
	...
	for (int i = start_v_page; i < start_v_page + page_num; i++) {
		v_page[i] = 1;
	}
	...
	for (int i = 0; i < USE_RECORD_SIZE; i++) {
...
int free(v_address addr, m_pid pid) {
	...
	unsigned int use_page_number = (use.size+PAGE_SIZE-1) / PAGE_SIZE;
	for (int i = start_page_id; i <start_page_id+use_page_number) {
		v_page[i] = 0;
		page_table[i].p_id = 0;
	}
	return 0;
